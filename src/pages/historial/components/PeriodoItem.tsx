...
interface Props {
  periodo: string;
  asignaturas: any[];
  isLast: boolean; // Para controlar la línea de tiempo visual
}
...
export const PeriodoItem = ({ periodo, asignaturas, isLast }: Props) => {
  // Cálculos locales para el header de la tarjeta
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-100 text-xs uppercase tracking-wider text-slate-400 bg-white">
                ...
                <th className="px-6 py-3 font-semibold">Asignatura / NRC</th>
            ...
            <tbody className="text-sm">
              {asignaturas.map((r, i) => (
                <tr key={i} className="hover:bg-slate-50/50 transition-colors border-b border-slate-50 last:border-0">
                  <td className="px-6 py-4 font-mono text-slate-500 font-medium">{r.codigo}</td>
                  <td className="px-6 py-4">
                     <p className="font-bold text-slate-700">{r.codigo}</p> {/* Aquí iría nombre si lo tuvieras */}
                     <p className="text-xs text-slate-400">NRC: {r.nrc}</p>
                  </td>
